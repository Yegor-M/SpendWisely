...
export function SummaryCards({ data }: { data: Summary }) {
  const cards = [
    { title: "Total Income",      value: fmt(data.total_income),          sub: `Avg ${fmt(data.avg_monthly_income)}/mo` },
    ...
    { title: "Savings Rate",      value: pct(data.savings_rate_pct),       sub: data.net_balance >= 0 ? "Stay on track" : "Spending > income" },
  ];
